...
{
    public class LocalizationDriver : GameDriverBase
    {
        public override int Priority { get; }

        public override async UniTask InitAsync()
        {
            m_Language = GameComponent.Base.EditorResourceMode && GameComponent.Base.EditorLanguage != Language.Unspecified ? GameComponent.Base.EditorLanguage : SystemLanguage;
        ...
        public void ChangeLanguage(Language language)
        {
            ...
            Log.Info("language changed : {0} ---> {1}", m_Language, language);
            m_Language = language;
        }
    }
}